...
element_s* OLU_CP_ABEEaVT::H1(element_s *e) {
...
unsigned char* OLU_CP_ABEEaVT::H2(element_s *e) {
    unsigned char *bytes = (unsigned char*)malloc(gt_length);
    element_to_bytes(bytes, e);
...
element_s* OLU_CP_ABEEaVT::computeXdelte(Ciphertext_CET *ciphertext, SecretKey *key_x, string pre_s, string post_s) {
    // compute wi
    utils util;
...
element_s* OLU_CP_ABEEaVT::computeV(Ciphertext_CET *ct, Key *sp_ch, Key *pk_ch, element_s *r_ch, element_t_matrix *M,
...
vector<Key*>* OLU_CP_ABEEaVT::setUp() {
    chamhash *ch = new chamhash();
    Key *sp = ch->setup();
...
vector<SecretKey*>* OLU_CP_ABEEaVT::keyGen(Key *public_key, Key *master_key, vector<string> *attributes) {
...
vector<SecretKey*>* OLU_CP_ABEEaVT::trapdoor(vector<SecretKey*> *secret_key) {
    vector<SecretKey*> *res = new vector<SecretKey*>(2);
...
Ciphertext_CET* OLU_CP_ABEEaVT::encrypt(Key *public_key, access_structure *A, unsigned char *message, Key *sp_ch, Key *pk_ch) {
...
Ciphertext_CET* OLU_CP_ABEEaVT::transform(Key *public_key, SecretKey *key_x, string *key_type, Ciphertext_CET *CT, Key *sp_ch, Key *pk_ch) {
...
bool* OLU_CP_ABEEaVT::test(Key *public_key, Ciphertext_CET *ITA, vector<SecretKey *> *TdSA, Ciphertext_CET *ITB,
...
unsigned char* OLU_CP_ABEEaVT::decrypt(Key *public_key, Ciphertext_CET *IT, SecretKey *DK) {
    if (IT == NULL) {
        return NULL;
    }
