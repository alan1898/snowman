...
element_s* DETC::computeXsub(Ciphertext_CET *ciphertext, SecretKey *key_x, string post_s) {
    // compute wi
    utils util;
...
vector<Key*>* DETC::setUp(vector<string> *attributes) {
    // randomly choose g
    element_t g;
...
Ciphertext_CET* DETC::encrypt(Key *public_key, access_structure *A, unsigned char *message) {
...
SecretKey* DETC::keyGen(Key *public_key, Key *master_key, vector<string> *attributes) {
...
SecretKey* DETC::trapdoor(Key *public_key, Key *master_key, vector<string> *attributes) {
    SecretKey *res = new SecretKey(attributes);
...
bool* DETC::test(Key *public_key, Ciphertext_CET *CTA, SecretKey *TdSA, Ciphertext_CET *CTB, SecretKey *TdSB) {
