...
element_s* HCET::computeXdelte(Ciphertext_HCET *ciphertext, SecretKey *key_x, string pre_s, string post_s) {
    // get structure
...
element_s* HCET::computeVj(Ciphertext_HCET *ct, Key *sp_ch, Key *pk_ch, element_s *r_ch, access_structure *structure) {
    element_t *res = new element_t[1];
...
SecretKey* HCET::userKeyGen(Key *public_key, Key *SKID, element_t_vector *ID, string *kgc_name, vector<string> *attributes) {
...
SecretKey* HCET::trapdoor(SecretKey *secret_key) {
    SecretKey *res = new SecretKey(secret_key->getAttributes(), secret_key->getKgcName());
...
Ciphertext_HCET* HCET::encrypt(Key *public_key, map<string, access_structure*> *AA, unsigned char *message, Key *sp_ch, Key *pk_ch) {
...
bool* HCET::test(Key *public_key, Ciphertext_HCET *CTA, SecretKey *TdSA, Ciphertext_HCET *CTB, SecretKey *TdSB, Key *sp_ch, Key *pk_ch) {
    // obtain public parameters
    element_t g, u, h, w, v;
    element_init_same_as(g, public_key->getComponent("g"));
    element_set(g, public_key->getComponent("g"));
...
unsigned char* HCET::decrypt(Key *public_key, Ciphertext_HCET *ciphertext_hcet, SecretKey *secret_key) {
    ...
    for (signed long int i = 0; i < 8; ++i) {
        int mzvalue = (int)ciphertext_hcet->Cstar[i] ^ (int)H_2[i];
...
//    element_printf("恢复出来的z为：%B\n", z);
    ...
    // obtain C0'
    element_t C0_;
    element_init_same_as(C0_, ciphertext_hcet->getComponent("C0_"));
    element_set(C0_, ciphertext_hcet->getComponent("C0_"));
    ...
    // obtain C
    element_t C;
    element_init_same_as(C, ciphertext_hcet->getComponent("C"));
